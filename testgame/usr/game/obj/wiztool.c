...
void upgraded(varargs int clone) {
  ...
  if(!SYSTEM() && previous_program() != GAME_WIZTOOL)
    error("Only System code may call upgraded!");
...
static void message(string str)
{
  if(user)
    ...
    LOGD->write_syslog("From master /usr/game/obj/wiztool.c: " + str,
		       LOG_ERROR);
}
...
void command(string cmd, string str) {
  if(previous_object() == user) {
    ...
    error("Only privileged code may call command()!");
}
...
static void process_command(string cmd, string str)
{
  ...
  command_sets = GAME_WIZTOOL->get_command_sets(this_object());
  ...
  default:
    message("No command: " + (cmd ? cmd : "(nil)") + "\n");
    break;
  }
}
